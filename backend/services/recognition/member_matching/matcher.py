...
class ParliamentMemberMatcher:
    ...
    def load_parliament_members(self) -> bool:
        """
        Load parliament members from Supabase or cache
        ...
        try:
            ...
            try:
                self.members = load_members_from_supabase(self.supabase)
                
                # Save to cache if successful
                if self.members:
                    save_members_to_cache(self.members, self.member_cache_file)
            except Exception as e:
                logger.error(f"Error loading members from Supabase: {str(e)}")
                logger.info("Falling back to cached members")
            
            # Fall back to cache if Supabase failed or returned no members
            if not self.members:
                self.members = load_members_from_cache(self.member_cache_file)
            
            if not self.members:
                logger.error("Failed to load parliament members from Supabase or cache")
                return False
