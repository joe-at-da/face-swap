...
class OptimizedFaceDetector:
    ...
    def detect_faces_in_roi(self, frame, roi_scale=0.6):
        ...
        if validated_locations:
            logger.debug(f"Extracting encodings for {len(validated_locations)} validated face locations")
            try:
                face_encodings = face_recognition.face_encodings(rgb_frame, validated_locations)
                logger.debug(f"Successfully extracted {len(face_encodings)} face encodings")
                if len(face_encodings) != len(validated_locations):
                    logger.warning(f"Encoding count mismatch: {len(validated_locations)} locations but {len(face_encodings)} encodings")
            except Exception as e:
                logger.error(f"Failed to extract face encodings: {str(e)}")
                face_encodings = []
        else:
            logger.debug("No validated face locations found for encoding extraction")
                ...
                if current_frame_idx % frame_interval == 0:
                    ...
                    if current_frame_idx == 0 or scene_changed or current_frame_idx % detection_interval == 0:
                        ...
                        for i, (face_loc, face_encoding) in enumerate(zip(face_locations, face_encodings)):
                            ...
                            logger.debug(f"Storing face {face_id} with encoding of length {len(face_encoding)} at timestamp {timestamp:.2f}")
