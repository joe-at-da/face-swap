...
def extract_audio_for_capture(
    ...
    This endpoint runs the extract_audio_standalone.py script for a specific capture ID.
    """
    try:
        # Path to the extract_audio_standalone.py script
        script_path = os.path.join("/app", "scripts", "extract_audio_standalone.py")
        ...
        def extract_audio():
            subprocess.run([sys.executable, script_path, str(capture_id)], check=False)
        
        # Add the task to the background tasks
        background_tasks.add_task(extract_audio)
