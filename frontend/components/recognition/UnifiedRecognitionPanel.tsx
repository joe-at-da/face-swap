...
// Component interfaces for tab views
...
// Placeholder components for tab views
const EnhancedView: React.FC<EnhancedViewProps> = ({ captureId, audioInfo, transcriptionData, integratedTimeline }) => {
  return (
    <div className="p-4 bg-gray-900 rounded-lg">
      <h3 className="text-lg font-semibold mb-4">Enhanced View</h3>
      <p className="text-gray-400">Enhanced view is under development. Please use the Unified View tab.</p>
    </div>
  );
};
...
const FacesView: React.FC<FacesViewProps> = ({ recognitionResults }) => {
  return (
    <div className="p-4 bg-gray-900 rounded-lg">
      <h3 className="text-lg font-semibold mb-4">Faces</h3>
      <p className="text-gray-400">Face recognition data will be displayed here.</p>
    </div>
  );
};
...
const TimelineView: React.FC<TimelineViewProps> = ({ videoId, transcriptionData, integratedTimeline }) => {
  return (
    <div className="p-4 bg-gray-900 rounded-lg">
      ...
      <p className="text-gray-400">Timeline data will be displayed here.</p>
    </div>
  );
};
...
const UnifiedRecognitionPanel: React.FC<UnifiedRecognitionPanelProps> = ({
  ...
  const [integratedTimeline, setIntegratedTimeline] = useState<any>(null);
  ...
  const startRecognitionProcess = async () => {
    try {
      ...
      // First, get audio info to ensure the capture has audio
      await fetchAudioInfo();
      
      // Start recognition process
      const response = await api.post(`/recognition/start-recognition/${captureId}`, {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        setError('Failed to start recognition process');
      }
    } catch (error: unknown) {
      console.error('Error starting recognition:', error);
      
      // Handle different error scenarios
      let errorMessage = 'Failed to start recognition process. Please try again.';
      ...
      if (axiosError?.response) {
        ...
        if (axiosError.response.status === 400) {
          errorMessage = 'Invalid request: ' + (axiosError.response.data?.message || 'Bad request');
          ...
          errorMessage = 'Server error: ' + (axiosError.response.data?.message || 'Please try again later');
        }
      ...
      <div className="mb-6">
        <div className="flex items-center mb-2">
          ...
          <span className={`px-2 py-1 rounded-full text-sm ${getStatusBgColor(recognitionStatus?.status || 'not_started')} ${getStatusColor(recognitionStatus?.status || 'not_started')}`}>
            {recognitionStatus?.status && typeof recognitionStatus.status === 'string' ? recognitionStatus.status.replace('_', ' ') : 'Not Started'}
          </span>
        </div>
