-- Always use PGMQ queues for embedding generation
-- Replaces the complex bulk detection system with a simple, reliable queue-based approach
...
BEGIN
    ...
    IF table_name_param = 'parliament_member_clips' THEN
        FOR clip_record IN
            ...
            AND id NOT IN (
                SELECT (message->>'clip_id')::uuid 
                FROM pgmq.metrics_all() m
                JOIN pgmq.read('embedding_jobs', 0, 1000) q ON true
                WHERE q.message->>'table_name' = 'parliament_member_clips'
            )
            LIMIT limit_param
        LOOP
