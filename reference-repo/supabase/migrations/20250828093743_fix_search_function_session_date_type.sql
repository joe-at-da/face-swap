-- Fix the search_user_clips_by_embedding function to use correct session_date type
...
RETURNS TABLE (
  ...
  transcript text,
  session_date date,  -- Fixed type from text to date
  session_type text,
  debate_topic text,
...
'Function to search user clips using embedding similarity through joined parliament_member_clips. Currently uses text similarity as fallback until proper embedding search is implemented. Fixed watermark_position and session_date types.';

-- Log migration completion
DO $$
BEGIN
    RAISE NOTICE 'Fixed search_user_clips_by_embedding function with correct session_date type';
END;
$$;