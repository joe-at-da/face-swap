-- Add 10 second delay to the trigger function before calling the endpoint
...
BEGIN
    ...
    -- Log that we're triggering the embedding generation with delay
    INSERT INTO transcript_embedding_logs (
        table_name, 
        clip_id, 
        status, 
        ...
        'triggered',
        transcript_len,
        'Waiting 10 seconds before sending request to generate embedding'
    );
    
    -- Wait 10 seconds before making the HTTP request
    PERFORM pg_sleep(10);
    ...
    BEGIN
        ...
        SET status = 'success', 
            ...
            notes = 'HTTP request sent successfully after 10s delay (async processing)'
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            SET status = 'failed', 
                ...
                notes = 'Failed to send HTTP request after 10s delay'
            WHERE table_name = TG_TABLE_NAME 
            AND clip_id = NEW.id 
            AND status = 'triggered'
            AND created_at = (
...
DO $$
BEGIN
    RAISE NOTICE 'Updated generate_transcript_embedding() function to include 10 second delay';
    RAISE NOTICE 'The function will now wait 10 seconds before calling the embedding API endpoint';
    RAISE NOTICE 'This prevents immediate API calls and provides a buffer for subsequent operations';
END;
$$;