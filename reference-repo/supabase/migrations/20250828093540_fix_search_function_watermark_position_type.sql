-- Fix the search_user_clips_by_embedding function to use correct watermark_position type
...
RETURNS TABLE (
  ...
  watermark_url text,
  watermark_position watermark_position,  -- Fixed type
  duration text,
...
'Function to search user clips using embedding similarity through joined parliament_member_clips. Currently uses text similarity as fallback until proper embedding search is implemented. Fixed watermark_position type.';

-- Log migration completion
DO $$
BEGIN
    RAISE NOTICE 'Fixed search_user_clips_by_embedding function with correct watermark_position type';
END;
$$;