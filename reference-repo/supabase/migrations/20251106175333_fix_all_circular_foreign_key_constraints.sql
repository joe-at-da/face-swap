-- Fix All Circular Foreign Key Constraints for pg_dump
-- This migration makes ALL foreign key constraints deferrable to avoid pg_dump warnings
-- about circular dependencies during backup operations.
-- Deferrable constraints allow constraint checks to be deferred until transaction commit,
-- enabling pg_dump to restore data in any order while maintaining referential integrity.
...
-- Note: This was changed to SET NULL in a later migration, but we need to make it deferrable
...
-- This was already made deferrable in the previous fix, but let's ensure it's still correct
...
-- ============================================================================
-- PARLIAMENT MEMBER FACE ENCODINGS FOREIGN KEYS
-- ============================================================================
...
DO $$
BEGIN
    RAISE NOTICE 'Circular foreign key constraints fix completed:';
