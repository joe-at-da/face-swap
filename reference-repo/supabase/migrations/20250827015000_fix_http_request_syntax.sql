-- Fix HTTP request syntax in process_embedding_queue function
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            ...
            -- Make HTTP POST request (correct syntax without timeout parameter)
            PERFORM http(
                'POST',
            ...
            -- If we get here, assume success (http extension doesn't return status directly)
            UPDATE transcript_embedding_logs 
            SET status = 'success',
                response_status = 200,
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ...
                -- Delete the job if it's a critical error
                IF SQLERRM LIKE '%timeout%' OR SQLERRM LIKE '%connection%' OR SQLERRM LIKE '%cast%' THEN
                    PERFORM pgmq.delete('embedding_jobs', job_record.msg_id);
                END IF;
...
DO $$
BEGIN
    RAISE NOTICE 'Fixed HTTP request syntax:';
    RAISE NOTICE '- Corrected http() function call syntax';
    RAISE NOTICE '- Removed unsupported timeout parameter';
    RAISE NOTICE '- Delete jobs with cast errors to prevent retry loops';
END;
$$;