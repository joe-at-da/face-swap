-- Fix hybrid_search_parliament_clips_all:
-- 1. Add semantic similarity threshold >= 0.10 (matches hybrid_search_parliament_clips)
-- 2. Switch from plainto_tsquery + manual OR to websearch_to_tsquery (matches working function)
-- Without the threshold, the semantic CTE returns all clips with any embedding,
-- flooding the reranker with irrelevant low-scoring candidates.
