-- Fix: Remove status = 'completed' filter from search functions
-- All clips have NULL status, so this filter was excluding every clip from AI/hybrid search results.
-- The page and text search already guard with is_deleted = false + transcript IS NOT NULL.

-- 1. Fix hybrid_search_parliament_clips
...
-- 2. Fix search_parliament_clips_by_vector
CREATE OR REPLACE FUNCTION public.search_parliament_clips_by_vector(
  query_embedding_text text,
  target_member_id integer,
