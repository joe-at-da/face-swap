...
export default function ContactError({ error, reset }: ErrorPageProps) {
  React.useEffect(() => {
    ErrorLogger.logClientError(
      ...
      {
        digest: error.digest,
        errorBoundary: 'contact-error-page',
      }
    );
  }, [error]);
  ...
  const errorMessage = process.env.NODE_ENV === 'development' 
    ...
    : 'An unexpected error occurred while loading the contact form. Please try again.';
      ...
      <Card className="w-full max-w-lg">
        ...
        <CardContent className="space-y-6 px-4 sm:px-6">
          ...
          <div className="text-center text-sm text-muted-foreground">
            <p>If this problem persists, please contact support directly.</p>
          </div>
        </CardContent>
      </Card>
    </div>
