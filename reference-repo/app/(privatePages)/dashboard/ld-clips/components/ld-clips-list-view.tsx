...
import LDClipsFilters from "./ld-clips-filters";
import AllClipCard from "@/app/(privatePages)/dashboard/all-clips/components/all-clip-card";
...
interface LDClipsListViewProps {
  teamId?: string;
}

const ITEMS_PER_PAGE = 24;

export default function LDClipsListView({ teamId }: LDClipsListViewProps) {
  ...
  const fetchClips = useCallback(
    async (page: number) => {
      ...
      try {
        ...
        const res = await fetch("/api/clips/search-ld", {
          method: "POST",
    ...
    [searchTerm, searchType, selectedMemberIds, dateRange, teamId]
  );
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-foreground pt-4">All LD Clips</h1>
      ...
      {isLoading && searchTerm ? (
        ...
        <Card>
          <CardContent className="p-12 text-center">
            <p className="text-lg text-muted-foreground">
              {searchTerm || selectedMemberIds.length > 0 || dateRange.from
                ? "No clips found matching your criteria."
                : "No LD clips available."}
            </p>
            {searchTerm && (
              <p className="text-sm text-muted-foreground mt-2">
